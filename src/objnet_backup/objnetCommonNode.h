...
//    void sendGlobalMessage(StdAID aid);
    ...
    void sendRemoteMessage(unsigned char receiver, unsigned char oid, const ByteArray &ba = ByteArray(), unsigned char mac=macAuto);
    
    void sendServiceMessage(SvcOID oid, unsigned char mac=0xFF, const ByteArray &ba = ByteArray(), unsigned char receiver=0x7F);
...
//    ObjnetMessageEvent retranslateEvent() const {return mRetranslateEvent;}
    ...
    /*! Отправка сообщения.
        Отправляет сообщение с указанными параметрами устройству с физическим адресом mac.
        В режиме мастера в параметр mac необходимо указать физический адрес устройства.
        В режиме девайса параметр mac заполняется автоматически собственным физическим адресом узла сети.
    ...
    */
    void sendMessage(unsigned char oid, const ByteArray &ba = ByteArray(), unsigned char mac=macAuto);
};
