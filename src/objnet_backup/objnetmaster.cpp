...
void ObjnetMaster::parseServiceMessage(CommonMessage &msg)
{
    ...
    switch (oid)
    {
      case svcEcho:
        if (!dev)
            sendServiceMessage(svcHello, mac); // reset node's connection state
        else
        {
            if (!dev->mPresent)
            {
      ...
      case svcHello:
      {
        ...
        {                                                // если размер данных > 1, значит, это ретранслированный запрос
            ...
            sendServiceMessage(svcWelcome, mac, ba);     // отправл€ем сообщение с присвоенным адресом
            #ifndef __ICCARM__
        ...
        else
        {
            ...
            ByteArray ba;
            ba.append(netaddr);
            sendServiceMessage(welcomeCmd, mac, ba);     // отправл€ем сообщение с присвоенным адресом
        }
