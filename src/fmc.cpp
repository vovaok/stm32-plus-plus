...
void SDRAM::initialize(int bank)
{
    ...
    ModeReg modereg;
    modereg.burstLength = 1;
    modereg.burstType = 0; // sequential
    modereg.CASLatency = CASLatency;
    modereg.operatingMode = 0; // standard operation
    modereg.writeBurstMode = burstEnabled? 0: 1;
    fmc.sdramCmd(bank, Fmc::CmdModeLoadMode, *reinterpret_cast<uint32_t*>(&modereg));
    
    // calculate refresh rate according to the STM32 datasheet
    int refreshRate = refreshPeriod_ms * 1000 * frequency_MHz / (1 << rows) - 20;
...
//    initAddressPins(MAX(sdram->rows, sdram->columns));
    ...
    Gpio::config(2, Gpio::FMC_NBL0, Gpio::FMC_NBL1);
