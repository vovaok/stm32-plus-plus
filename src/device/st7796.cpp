...
ST7796::ST7796(Spi *spi, Gpio::PinName cs, Gpio::PinName dc, Gpio::PinName rst) :
    Display(),
    m_spi(spi),
...
void ST7796::setBacklightPin(Gpio::Config pin)
{
    m_pwmPin = pin;
...
void ST7796::setBacklight(int percent)
{
...
void ST7796::writeCmd(uint8_t cmd, uint8_t *data, int size)
{
    writeCmd(cmd);
    while (size--)
        writeData(*data++);
...
void ST7796::writeCmd(uint8_t cmd, int size, ...)
{
    writeCmd(cmd);
    va_list vl;
    va_start(vl, size);
...
void ST7796::copyRect(int x, int y, int width, int height, const uint8_t *buffer)
{
    setArea(x, y, x+width-1, y+height-1);
    // Memory write start
    m_cs->reset();
...
void ST7796::overlayRect(int x, int y, int width, int height, uint32_t color)
{
    fillRect(x, y, width, height, color); // no blending
}

void ST7796::blendRect(int x, int y, int width, int height, const uint8_t *buffer, PixelFormat format)
{
...
void ST7796::drawBuffer(int x, int y, const FrameBuffer *fb, int sx, int sy, int sw, int sh)
{
